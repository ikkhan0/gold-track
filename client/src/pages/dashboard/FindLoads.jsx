            ...
            <div className="flex-1 overflow-y-auto pr-2">
                {loading ? <div className="text-center p-10">Loading loads...</div> : (
                    <div className="grid gap-4">
                        ...
                        {loads.map((load) => (
                            <Card key={load._id} className="hover:shadow-md transition-shadow border-l-4 border-l-emerald-500">
            ...
            <Modal title={`Bid on Load`} isOpen={bidModalOpen} onClose={() => setBidModalOpen(false)}>
                <div className="space-y-4">
                    <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p className="mt-2 text-emerald-600 font-bold">Shipper Offer: {selectedLoad?.offerPrice ? `Rs ${selectedLoad.offerPrice}` : 'Open'}</p>
                    </div>
                    <Input label="Your Bid Amount (Rs)" type="number" placeholder="e.g. 90000" />
                    <Input label="Note (Optional)" placeholder="I can pick it up tomorrow..." />
